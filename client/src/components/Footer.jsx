...
const Footer = () => {
  ...
  const handleLinkClick = (e, link) => {
    // Prevent the default link behavior for all clicks handled by this function
    e.preventDefault();
          ...
          <div>
            ...
            <ul className="space-y-3">
              {footerLinks[1].links.map((link) => (
                <li key={link.text}>
                  <a href={link.url} onClick={(e) => handleLinkClick(e, link)} className="text-sm text-stone-500 hover:text-amber-700 transition-colors cursor-pointer">
                    {link.text}
                  </a>
                </li>
              ))}
